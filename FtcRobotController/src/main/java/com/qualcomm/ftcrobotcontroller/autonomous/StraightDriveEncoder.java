 ...
 */
public class StraightDriveEncoder extends AutonomousCommand{

	private static final int TICKS_PER_INCH = 0;
	private static final double SCALER = 10;
	...
	public StraightDriveEncoder(int distance, DriveTrainController driveTrain, Telemetry telem){
		super(telem);
	...
	public void update(float delta) {
		...
		if(isLeftComplete()){
			...
			driveTrain.setLeftPower((distance - leftPos+ 0.0)/distance - flux/distance*SCALER);
		}
		if(isRightComplete()){
			driveTrain.setRightPower(0);
		}else{
			driveTrain.setRightPower((distance - rightPos+ 0.0)/distance + flux/distance*SCALER);
		}
